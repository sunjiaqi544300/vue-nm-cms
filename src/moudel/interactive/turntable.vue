<template>
    <div class="container">
    <!-- <div v-for="(item,index) in act_award" :key="index">
   {{item.award.name}}
    </div> -->
        <div class="lucky-wheel" :style="{backgroundImage: 'url(' + bgpic + ')'}" >
            <!-- <div class="lucky-title"></div> -->
            <div class="wheel-main">
                <div class="wheel-pointer-box">
                     <div class="wheel-pointer" @click="rotate_handle()" :style="{backgroundImage: 'url(' + arrpic + ')',transform:rotate_angle_pointer,transition:rotate_transition_pointer}"></div>
                </div>               
                <div class="wheel-bg" :style="{transform:rotate_angle,transition:rotate_transition,backgroundImage: 'url(' + turpic + ')'}">    
                	<!-- <ul id="wheel" class="wheel-list" style="transform: rotate(0deg);">
                  </ul>                -->
                    <div class="prize-list">
                        <div class="prize-item" v-for="(item,index) in act_award" :key="index" :style='{transform: `rotate(${index*num}deg`}'>
                            <div class="prize-pic">
                                <img :src="item.award.pic">
                            </div>
...
export default {
  ...
  mounted () {
     let vm = this;
      vm.$axios
        .get("/mprogram/api/activity/?s_id=22")
        .then(function (response) {
          console.log(response.data[0].activity.act_award)
          // console.log(response.data[0].activity.act_award.length)
          vm.num =  360/response.data[0].activity.act_award.length
        //  console.log( vm.num)
          vm.act_award  = response.data[0].activity.act_award
          vm.bgpic = response.data[0].activity.bgpic
          vm.turpic = response.data[0].activity.turpic
          vm.arrpic = response.data[0].activity.arrpic
  ...
  methods: {
    ...
    rotate_handle() {
      this.index = 1 //指定每次旋转到的奖品下标
       let vm = this;
       vm.$axios
        .get( "/mprogram/api/lottery/?s_id=22")
        .then(function (response) {
          // console.log(response.data.award_id)
         vm.award_id = response.data.award_id
         for (let index = 0; index < vm.act_award.length; index++) {
           if ( vm.act_award[index].award.id ==  vm.award_id) {
               console.log(index)
               vm.angles =  index* vm.num 
               console.log(vm.angles)
               vm.integral =  vm.act_award[index].award.name
               vm.rotating();
              //  transform: `rotate(${index*num}deg
           }
    ...
    rotating() {
      ...
      // var result_angle = []; //最终会旋转到下标的位置所需要的度数
