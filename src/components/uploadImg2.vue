...
export default {
  ...
  data() {
    return {
      ...
      apihost: this.$apihost + "/wechat/api/head_img",
      //文件上传的参数
      dialogVisible: false,
      //图片列表（用于在上传组件中回显图片）
      fileList: [],
  ...
  methods: {
    getdata() {
      this.$axios
        .get(
          `${this.$apihost}` + "/wechat/api/head_img/?search=" + this.Dlicense
        )
        .then(result => {
          if (result.data) {
    ...
    uploadUrl() {
      return `${this.$apihost}` + "/wechat/api/head_img/";
    },
    isBase64(str) {
      ...
      if (isJPG || isBMP || isGIF || isPNG) {
        ...
        for (let index = 0; index < fileList.length; index++) {
          if (fileList[index].raw.size / 1024 / 1024 < 2) {
            // console.log("图片正常")
            fd.append("head_img", fileList[index].raw);
            self.loading = false;
          } else {
            //判断支不支持FileReader
    ...
    subFn() {
      ...
      // console.log(this.fd)
      axiosInstance
        .post(`${this.$apihost}` + "/wechat/api/head_img/", this.fd, config)
        .then(result => {
          if (result.data == "上传成功") {
    ...
    beforeUpload(file) {
      ...
      this.param.append("head_img", file, file.name);
      return false;
